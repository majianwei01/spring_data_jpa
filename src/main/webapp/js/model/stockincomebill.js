$(function () {
    ...
    //拿到页面中常用的一些组件
    //拿到有用户信息的组件
    var stockincomebillGrid = $("#stockincomebillGrid");
    var searchForm = $("#searchForm");
    var stockincomebillDlg=$("#stockincomebillDlg");
    var stockincomebillForm=$("#stockincomebillForm");
    ...
    itsource = {
        add(){
            //把表单进行清空
            stockincomebillForm.form("clear");
            //把一些有data-hide的属性元素显示起来
            $("*[data-hide]").show();
            $("*[data-hide] input").validatebox("enable");
            //显示咱们的表单窗口
            stockincomebillDlg.dialog("center").dialog("open");
        },
        ...
        save(){
            ...
            if(id){
                url = "/stockincomebill/update?_cmd=update";
                console.debug("56789");
            }
            //用easyui(form)提供的方案进行提交
            // 自动拿到所有数据，并且使用Ajax的方式提交
            stockincomebillForm.form('submit', {
                url:url,
                //提交之前做的事情(返回false就不会提交)
                onSubmit: function(){
                    // 一般在这里面做验证功能
                    return stockincomebillForm.form("validate");
                },
                ...
                success:function(data){
                    ...
                    if(result.success){
                        //关闭窗口
                        stockincomebillDlg.dialog("close");
                        //刷新grid
                        stockincomebillGrid.datagrid("reload");
                    }else{
        ...
        delete(){
            //1.获取选中的是哪一行数据
            var row = stockincomebillGrid.datagrid("getSelected");
            if(row){
                //3.用户选择是否要进行删除
                $.messager.confirm('提示','确认删除这行数据吗？',function(r){
                    if (r){
                        ...
                        $.get("/stockincomebill/delete",{id:row.id},function(result){
                            ...
                            if(result.success){
                                //6.删除成功的话,页面进行刷新
                                stockincomebillGrid.datagrid("load");
                            }else{
                                $.messager.alert('提示','删除失败! 原因:'+result.msg,'error');
                            }
                        })
        ...
        update(){
            ...
            if(row){
                ...
                //}
                //进行数据的回显(在清空后面)
                stockincomebillForm.form("load",row);
                //弹出界面
                stockincomebillDlg.dialog("center").dialog("open");
            }else{
        ...
        search(){
            ...
            //根据查询的值刷新grid数据
            stockincomebillGrid.datagrid("load",params);
        }
    };
