$(function () {
    ...
    //拿到页面中常用的一些组件
    //拿到有用户信息的组件
    var stockincomebillitemGrid = $("#stockincomebillitemGrid");
    var searchForm = $("#searchForm");
    var stockincomebillitemDlg=$("#stockincomebillitemDlg");
    var stockincomebillitemForm=$("#stockincomebillitemForm");
    ...
    itsource = {
        add(){
            //把表单进行清空
            stockincomebillitemForm.form("clear");
            //把一些有data-hide的属性元素显示起来
            $("*[data-hide]").show();
            $("*[data-hide] input").validatebox("enable");
            //显示咱们的表单窗口
            stockincomebillitemDlg.dialog("center").dialog("open");
        },
        ...
        save(){
            ...
            if(id){
                url = "/stockincomebillitem/update?_cmd=update";
                console.debug("56789");
            }
            //用easyui(form)提供的方案进行提交
            // 自动拿到所有数据，并且使用Ajax的方式提交
            stockincomebillitemForm.form('submit', {
                url:url,
                //提交之前做的事情(返回false就不会提交)
                onSubmit: function(){
                    // 一般在这里面做验证功能
                    return stockincomebillitemForm.form("validate");
                },
                ...
                success:function(data){
                    ...
                    if(result.success){
                        //关闭窗口
                        stockincomebillitemDlg.dialog("close");
                        //刷新grid
                        stockincomebillitemGrid.datagrid("reload");
                    }else{
        ...
        delete(){
            //1.获取选中的是哪一行数据
            var row = stockincomebillitemGrid.datagrid("getSelected");
            if(row){
                //3.用户选择是否要进行删除
                $.messager.confirm('提示','确认删除这行数据吗？',function(r){
                    if (r){
                        ...
                        $.get("/stockincomebillitem/delete",{id:row.id},function(result){
                            ...
                            if(result.success){
                                //6.删除成功的话,页面进行刷新
                                stockincomebillitemGrid.datagrid("load");
                            }else{
                                $.messager.alert('提示','删除失败! 原因:'+result.msg,'error');
                            }
                        })
        ...
        update(){
            ...
            if(row){
                ...
                //}
                //进行数据的回显(在清空后面)
                stockincomebillitemForm.form("load",row);
                //弹出界面
                stockincomebillitemDlg.dialog("center").dialog("open");
            }else{
        ...
        search(){
            ...
            //根据查询的值刷新grid数据
            stockincomebillitemGrid.datagrid("load",params);
        }
    };
