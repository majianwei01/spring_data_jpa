...
import com.majianwei.query.PermissionQuery;
...
public class PermissionController extends BaseController {
    ...
    //   /WEB-INF/views/emp/permission.jsp 浏览器真实访问路径
    public String index() {
    ...
    @RequestMapping("/page")
    @ResponseBody
    public List<Permission> showData(){
        List<Permission> permission = permissionService.queryAll();
    ...
    @ResponseBody
    public UIPage page2(PermissionQuery query){
//        Page<Permission> page = permissionService.findPageByQuery(query);
//        UIPage uiPage = new UIPage(page);
        System.out.println(permissionService);
        Page pageByQuery = permissionService.findPageByQuery(query);
        //将查询出的数据封装到UIpage
    ...
    public JsonResult save(Permission permission){
        JsonResult jsonResult = saveOrUpdate(permission);
        return jsonResult;
    }
...
//            //代表是修改
//            //1.根据id到数据库中查询员工
//            Permission dbEmp = permissionService.queryOne(id);
//            //2.如果名称(数据库的员工名称与传过来的)相等,就直接返回true
//            if(dbEmp.getUsername().equals(username)){
//                return true;
//            }
//        }
//        return permissionService.checkName(username);
//    }
